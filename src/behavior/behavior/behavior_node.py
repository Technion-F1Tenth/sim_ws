...
class Behavior(Node):
    ...
    def init_poses(self):
        base_waypoints = np.genfromtxt(home+'/sim_ws/wps/'+str(self.get_parameter('file_name').get_parameter_value().string_value), delimiter=',')
        ...
        if self.poses is not None and self.odometry is not None:
            ...
            try: 
                ...
                for index_pose, pose in enumerate(self.poses):
                    ...
                    if x_grid >= 0 and x_grid < message.info.height and y_grid >= 0 and y_grid < message.info.width:
