...
class OccupancyGridNode(Node):
    ...
    def scan_callback(self, scan_msg):
        ...
        if self.car_position is not None and self.map_metadata is not None:
            ...
            grid = np.ndarray((self.height, self.width), buffer=np.zeros((self.height, self.width), dtype=np.int), dtype=np.int)
            grid.fill(int(-1))
    ...
    def generate_metadata(self):
        ...
        (roll, pitch, yaw) = euler_from_quaternion([self.car_position.pose.pose.orientation.x,self.car_position.pose.pose.orientation.y,self.car_position.pose.pose.orientation.z,self.car_position.pose.pose.orientation.w])



        car_theta = yaw # Implement this method to get the car's orientation (yaw angle)
        ...
        origin_x = car_x - map_center_x * md.resolution * math.cos(car_theta) + map_center_y * md.resolution * math.sin(car_theta)
        origin_y = car_y - map_center_x * md.resolution * math.sin(car_theta) - map_center_y * md.resolution * math.cos(car_theta)
